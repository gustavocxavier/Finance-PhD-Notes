...
library(dplyr)
setwd("C:\\Users\\HCBUser\\Downloads\\Data_R_WRDS")
...
data.comp <- data.ccm %>%
  ...
  group_by(PERMNO) %>% 
  mutate(datadate = zoo::as.yearmon(datadate)) %>%
  mutate(comp.count = n()) %>% # number of years in data; future option to cut first year data; works but leads to warnings
...
# as.data.table(data.comp)[lpermno==91914, .(gvkey, lpermno, comp.count)]
...
# #   Failed to fetch row: ERROR:  permission denied for schema crsp_q_stock
# 
...
data.crsp.m <- crsp.msf %>%
  ...
  mutate(retadj=ifelse(!is.na(ret), ret, NA)) %>% # create retadj
  # mutate(retadj=ifelse(!is.na(ret), ret, ifelse(!is.na(dlret), dlret, NA))) %>% # create retadj by merging ret and dlret
  # #RESOLVER: Error in ifelse(!is.na(dlret), dlret, NA) : object 'dlret' not found
  arrange(Date, PERMNO) %>%
  group_by(PERMNO) %>%    
